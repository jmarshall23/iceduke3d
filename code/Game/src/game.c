...
void printCenteredWithBackground(const char* str, int row) {
    int width = getConsoleWidth();
    int len = strlen(str);
    int padding = (width - len) / 2;

    // Move the cursor to the start of the specified row
    printf("\033[%d;1H", row);

    // Print the red background with the text centered
    setRedBackground();
    for (int i = 0; i < width; ++i) {
        if (i < padding || i >= padding + len) {
            printf(" "); // Print space with red background
        }
        else {
            printf("%c", str[i - padding]); // Print character with red background
        }
    }
    printf("\n");
}
...
int main(int argc,char  **argv)
{
    ...
    printCenteredWithBackground("Copyright (c) 1996 3D Realms Entertainment", 2);
    printf("\n");
...
void takescreenshot(void)
{
	...
	sprintf(text, "DukeNukem3D(v%d.%d) %.4d.%.2d.%.2d %.2dh%.2dm%.2ds", 
